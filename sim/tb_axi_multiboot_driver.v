...
module tb_axi_multiboot_driver_core #(
    ...
    parameter integer TB_POST_ERASE_STATUS_GUARD_NS = 0,
    parameter integer DUT_POST_ERASE_STATUS_GUARD_CYCLES = 10,
    parameter integer TB_FIRST_FLASH_TOTAL_BEATS = 16,
    parameter integer TB_RUN_SECOND_FLASH_CASE = 1,
    parameter integer TB_EXPECT_FIRST_FLASH_ERROR = 0,
    parameter integer TB_EXPECT_FIRST_FAIL_STAGE = 0
);
    ...
    localparam [31:0] DEVICE_ID              = 32'h0365_1093;
    ...
    task T_wait_flash_error_and_check;
        input [4:0] I_expect_fail_stage;
        reg [31:0] R_wait_cnt;
        begin
            R_wait_cnt = 32'd0;
            while ((!dut.W_err) && (!dut.W_drv_opt_ok) && (R_wait_cnt < MAX_WAIT_CYCLES)) begin
                @(posedge s00_axi_aclk);
                R_wait_cnt = R_wait_cnt + 1'b1;
            end

            if (R_wait_cnt >= MAX_WAIT_CYCLES)
                T_mark_error("flash ctrl error timeout");
            if (!dut.W_err)
                T_mark_error("flash ctrl expected error flag not seen");
            if (dut.W_drv_opt_ok)
                T_mark_error("unexpected flash ctrl done flag in error case");

            T_axi_read(32'h0000_0004, R_read_data);
            if (R_read_data[7:0] != 8'hAA)
                T_mark_error("feedback register != 0xAA in error case");
            if (R_read_data[10:8] == 3'b000)
                T_mark_error("feedback register error bits should be non-zero in error case");
            if (R_read_data[15:11] != I_expect_fail_stage)
                T_mark_error("feedback register last_fail_stage mismatch in error case");

            if (FLASH_MODEL == FLASH_MODEL_S25FL256S) begin
                if (W_clear_status_count != R_clear_status_start)
                    T_mark_error("S25 path should not issue clear status in error case");
            end
        end
    endtask
    ...
    tb_spi_flash_model #(
        ...
        .P_POST_ERASE_STATUS_GUARD_NS(TB_POST_ERASE_STATUS_GUARD_NS)
    ) u_spi_flash_model (
    ...
    initial begin
        ...
        R_total_beats = TB_FIRST_FLASH_TOTAL_BEATS;
        R_sent_beats  = 32'd0;
        R_run_stream  = 1'b1;
        R_stream_seed = 32'h10;
        R_wr_cnt_done_valid = 1'b0;
        ...
        if (FLASH_MODEL == FLASH_MODEL_S25FL256S)
            T_issue_flash_ctrl(32'h0000_1000, (R_total_beats * WR_DATA_MAX_LEN), 1'b0);
        else
            T_issue_flash_ctrl(32'h0001_0000, (R_total_beats * WR_DATA_MAX_LEN), 1'b0);

        if (TB_EXPECT_FIRST_FLASH_ERROR != 0)
            T_wait_flash_error_and_check(TB_EXPECT_FIRST_FAIL_STAGE[4:0]);
        else if (FLASH_MODEL == FLASH_MODEL_S25FL256S)
            T_wait_flash_done_and_check(R_total_beats, 1'b0, 32'h0000_1000, R_stream_seed);
        else
            T_wait_flash_done_and_check(R_total_beats, 1'b0, 32'h0001_0000, R_stream_seed);
        R_run_stream = 1'b0;
        repeat (30) @(posedge s00_axi_aclk);

        if (TB_RUN_SECOND_FLASH_CASE != 0) begin
            R_total_beats = 32'd16;
            R_sent_beats  = 32'd0;
            R_run_stream  = 1'b1;
            ...
            if (FLASH_MODEL == FLASH_MODEL_S25FL256S)
                T_issue_flash_ctrl(32'h0002_0000, (R_total_beats * WR_DATA_MAX_LEN), 1'b1);
            else
                T_issue_flash_ctrl(32'h0001_1000, (R_total_beats * WR_DATA_MAX_LEN), 1'b1);

            if (FLASH_MODEL == FLASH_MODEL_S25FL256S)
                T_wait_flash_done_and_check(R_total_beats, 1'b1, 32'h0002_0000, R_stream_seed);
            else
                T_wait_flash_done_and_check(R_total_beats, 1'b1, 32'h0001_1000, R_stream_seed);
            R_run_stream = 1'b0;
            repeat (30) @(posedge s00_axi_aclk);
        end
        ...
        if (R_error_cnt == 0) begin
            $display("=====================================================");
            $display("[TB][PASS] tb_axi_multiboot_driver_core pass (FLASH_MODEL=%0d)", FLASH_MODEL);
            $display("=====================================================");
        end
        else begin
            $display("=====================================================");
            $display("[TB][FAIL] tb_axi_multiboot_driver_core fail, error_cnt=%0d", R_error_cnt);
            $display("=====================================================");
        end
...
module tb_axi_multiboot_driver_s25;
    tb_axi_multiboot_driver_core #(
        .FLASH_MODEL           (0),
        .FPGA_FAMILY           (0),
...
module tb_axi_multiboot_driver_mt25;
    tb_axi_multiboot_driver_core #(
        .FLASH_MODEL           (1),
        .FPGA_FAMILY           (0),
        .MULTIBOOT_ADDR_SHIFT  (0)
...
module tb_axi_multiboot_driver_n25q128a;
    tb_axi_multiboot_driver_core #(
        .FLASH_MODEL           (2),
        .FPGA_FAMILY           (0),
        .MULTIBOOT_ADDR_SHIFT  (0)
...
module tb_axi_multiboot_driver_s25_post_erase_guard_fail;
    tb_axi_multiboot_driver_core #(
        .FLASH_MODEL                       (0),
        .FPGA_FAMILY                       (0),
        .MULTIBOOT_ADDR_SHIFT              (0),
        .TB_POST_ERASE_STATUS_GUARD_NS     (2000),
        .DUT_POST_ERASE_STATUS_GUARD_CYCLES(0),
        .TB_FIRST_FLASH_TOTAL_BEATS        (16),
        .TB_RUN_SECOND_FLASH_CASE          (0),
        .TB_EXPECT_FIRST_FLASH_ERROR       (1),
        .TB_EXPECT_FIRST_FAIL_STAGE        (10)
    ) u_core ();
endmodule

module tb_axi_multiboot_driver_s25_post_erase_guard_pass;
    tb_axi_multiboot_driver_core #(
        .FLASH_MODEL                       (0),
        .FPGA_FAMILY                       (0),
        .MULTIBOOT_ADDR_SHIFT              (0),
        .TB_POST_ERASE_STATUS_GUARD_NS     (2000),
        .DUT_POST_ERASE_STATUS_GUARD_CYCLES(200),
        .TB_FIRST_FLASH_TOTAL_BEATS        (16),
        .TB_RUN_SECOND_FLASH_CASE          (0),
        .TB_EXPECT_FIRST_FLASH_ERROR       (0),
        .TB_EXPECT_FIRST_FAIL_STAGE        (0)
    ) u_core ();
endmodule
