...
$incDir     = Join-Path $srcDir 'axis_data_fifo_32bytes\hdl'
...
$sourceFiles = @(
    ...
    (Join-Path $srcDir 'axis_data_fifo_32bytes\hdl\axis_infrastructure_v1_1_vl_rfs.v'),
    (Join-Path $srcDir 'axis_data_fifo_32bytes\hdl\axis_data_fifo_v2_0_vl_rfs.v'),
    (Join-Path $srcDir 'axis_data_fifo_32bytes\sim\axis_data_fifo_32bytes.v'),
    $glblFile,
    (Join-Path $scriptDir 'tb_spi_flash_model.v'),
    (Join-Path $scriptDir 'tb_axi_multiboot_driver_segmented_32b.v')
)

$tops = @(
    'tb_axi_multiboot_driver_segmented_32b_s25',
    'tb_axi_multiboot_driver_segmented_32b_mt25',
    'tb_axi_multiboot_driver_segmented_32b_n25q128a'
)
