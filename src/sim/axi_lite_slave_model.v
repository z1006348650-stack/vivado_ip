...
module axi_lite_slave_model #(
    ...
    input  wire [AXI_ADDR_WIDTH-1:0] i_s_axil_awaddr,
    input  wire [2:0]                i_s_axil_awprot,
    input  wire                      i_s_axil_awvalid,
    output reg                       o_s_axil_awready,
    input  wire [31:0]               i_s_axil_wdata,
    input  wire [3:0]                i_s_axil_wstrb,
    input  wire                      i_s_axil_wvalid,
    output reg                       o_s_axil_wready,
    output reg [1:0]                 o_s_axil_bresp,
    output reg                       o_s_axil_bvalid,
    input  wire                      i_s_axil_bready,
    input  wire [AXI_ADDR_WIDTH-1:0] i_s_axil_araddr,
    input  wire [2:0]                i_s_axil_arprot,
    input  wire                      i_s_axil_arvalid,
    output reg                       o_s_axil_arready,
    output reg [31:0]                o_s_axil_rdata,
    output reg [1:0]                 o_s_axil_rresp,
    output reg                       o_s_axil_rvalid,
...
always @(posedge i_clk) begin
    if (i_rst) begin
        ...
        o_s_axil_awready <= 1'b0;
        o_s_axil_wready  <= 1'b0;
        o_s_axil_bresp   <= 2'b00;
        o_s_axil_bvalid  <= 1'b0;
        o_s_axil_arready <= 1'b0;
        o_s_axil_rdata   <= 32'd0;
        o_s_axil_rresp   <= 2'b00;
        o_s_axil_rvalid  <= 1'b0;
        ...
        case (r_state)
            ST_IDLE: begin
                ...
                if (i_s_axil_awvalid) begin
                    r_state <= ST_WR_AW;
                end
                else if (i_s_axil_arvalid) begin
                    r_state <= ST_RD_AR;
                end
            end

            ST_WR_AW: begin
                if (o_s_axil_awready && i_s_axil_awvalid) begin
                    o_s_axil_awready <= 1'b0;
                    r_addr_latched     <= i_s_axil_awaddr;
                    r_wait_cnt         <= 8'd0;
                    r_state            <= ST_WR_W;
                end
                else if (!i_block_aw) begin
                    if (r_wait_cnt >= i_aw_wait_cycles) begin
                        o_s_axil_awready <= 1'b1;
                    end
                    else begin
            ...
            ST_WR_W: begin
                if (o_s_axil_wready && i_s_axil_wvalid) begin
                    o_s_axil_wready <= 1'b0;
                    r_mem_word[r_addr_latched[9:2]] <= i_s_axil_wdata;
                    o_s_axil_bresp  <= i_force_bresp_err ? 2'b10 : 2'b00;
                    r_wait_cnt        <= 8'd0;
                    r_state           <= ST_WR_B;
                end
                else if (!i_block_w) begin
                    if (r_wait_cnt >= i_w_wait_cycles) begin
                        o_s_axil_wready <= 1'b1;
                    end
                    else begin
                        r_wait_cnt <= r_wait_cnt + 8'd1;
                    end
            ...
            ST_WR_B: begin
                if (o_s_axil_bvalid && i_s_axil_bready) begin
                    o_s_axil_bvalid <= 1'b0;
                    r_wait_cnt        <= 8'd0;
                    r_state           <= ST_IDLE;
                end
            ...
            ST_RD_AR: begin
                if (o_s_axil_arready && i_s_axil_arvalid) begin
                    o_s_axil_arready <= 1'b0;
                    r_addr_latched     <= i_s_axil_araddr;
                    r_wait_cnt         <= 8'd0;
                    r_state            <= ST_RD_R;
                end
                else if (!i_block_ar) begin
                    if (r_wait_cnt >= i_ar_wait_cycles) begin
                        o_s_axil_arready <= 1'b1;
                    end
                    else begin
                        r_wait_cnt <= r_wait_cnt + 8'd1;
                    end
            ...
            ST_RD_R: begin
                if (o_s_axil_rvalid && i_s_axil_rready) begin
                    o_s_axil_rvalid <= 1'b0;
                    r_wait_cnt        <= 8'd0;
                    r_state           <= ST_IDLE;
                end
                    ...
                    if (r_wait_cnt >= i_r_wait_cycles) begin
                        o_s_axil_rvalid <= 1'b1;
                        o_s_axil_rdata  <= r_mem_word[r_addr_latched[9:2]];
                        o_s_axil_rresp  <= i_force_rresp_err ? 2'b10 : 2'b00;
                    end
                    else begin
                        r_wait_cnt <= r_wait_cnt + 8'd1;
                    end
