...
function integer calc_mem_addr_width;
    input integer value;
    integer width;
    integer tmp_value;
    begin
        ...
        if (width < 1) begin
            calc_mem_addr_width = 1;
        end
        else begin
            calc_mem_addr_width = width;
        end
    end
endfunction

localparam integer LP_MEM_ADDR_WIDTH = calc_mem_addr_width(MEM_WORDS);

reg [2:0] r_state;
reg [7:0] r_wait_cnt;
reg [AXI_ADDR_WIDTH-1:0] r_addr_latched;
reg [8:0] r_beats_latched;
reg [8:0] r_beat_idx;
        ...
        case (r_state)
            ...
            ST_WR_AW: begin
                if (o_s_axi_awready && i_s_axi_awvalid) begin
                    ...
                    r_beats_latched   <= {1'b0, i_s_axi_awlen} + 9'd1;
                    r_beat_idx        <= 9'd0;
                    r_wait_cnt        <= 8'd0;
                    r_state           <= ST_WR_W;
                end
            ...
            ST_WR_W: begin
                if (o_s_axi_wready && i_s_axi_wvalid) begin
                    o_s_axi_wready <= 1'b0;
                    r_mem_word[r_addr_latched[LP_MEM_ADDR_WIDTH+1:2] + r_beat_idx] <= i_s_axi_wdata;
                    r_wait_cnt <= 8'd0;

                    if (r_beat_idx == (r_beats_latched - 9'd1)) begin
                        o_s_axi_bresp <= i_force_bresp_err ? 2'b10 : 2'b00;
                        r_state         <= ST_WR_B;
                    end

                    r_beat_idx <= r_beat_idx + 9'd1;
                end
            ...
            ST_RD_AR: begin
                if (o_s_axi_arready && i_s_axi_arvalid) begin
                    ...
                    r_beats_latched   <= {1'b0, i_s_axi_arlen} + 9'd1;
                    r_beat_idx        <= 9'd0;
                    r_wait_cnt        <= 8'd0;
                    r_state           <= ST_RD_R;
                end
            ...
            ST_RD_R: begin
                if (o_s_axi_rvalid && i_s_axi_rready) begin
                    ...
                    if (r_wait_cnt >= i_r_wait_cycles) begin
                        o_s_axi_rvalid <= 1'b1;
                        o_s_axi_rdata  <= r_mem_word[r_addr_latched[LP_MEM_ADDR_WIDTH+1:2] + r_beat_idx];
                        o_s_axi_rresp  <= i_force_rresp_err ? 2'b10 : 2'b00;
                        o_s_axi_rlast  <= (r_beat_idx == (r_beats_latched - 9'd1));
                    end
                    else begin
                        r_wait_cnt <= r_wait_cnt + 8'd1;
                    end
