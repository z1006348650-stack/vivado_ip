...
localparam integer RX_TIMEOUT_CYCLES    = 250000;
...
reg [7:0] r_tx_buf [0:2047];
reg [7:0] r_data_buf [0:1023];
reg [7:0] r_exp_buf [0:2047];
reg [7:0] r_rx_buf [0:2047];
integer r_tx_count;
integer r_exp_count;
integer r_rx_count;
...
always @(posedge r_clk) begin
    if (!r_rst_n) begin
        r_frame_drop_count <= 0;
    end
    else if (dut.w_parser_frame_drop_pulse) begin
        r_frame_drop_count <= r_frame_drop_count + 1;
    end
end
...
task fill_stress_1024_data;
    input integer iter_idx;
    integer j;
    reg [7:0] r_iter_byte;
    begin
        r_iter_byte = iter_idx[7:0];
        for (j = 0; j < 256; j = j + 1) begin
            r_data_buf[j * 4]     = r_iter_byte + j[7:0];
            r_data_buf[j * 4 + 1] = (8'h31 + r_iter_byte) ^ j[7:0];
            r_data_buf[j * 4 + 2] = (8'h87 - r_iter_byte) + j[7:0];
            r_data_buf[j * 4 + 3] = (8'hC3 ^ r_iter_byte) - j[7:0];
        end
    end
endtask
...
task build_request;
    ...
    begin
        ...
        r_tx_buf[r_tx_count] = req_len[15:8];
        r_chk = r_chk ^ req_len[15:8];
        r_tx_count = r_tx_count + 1;

        r_tx_buf[r_tx_count] = req_len[7:0];
        r_chk = r_chk ^ req_len[7:0];
        r_tx_count = r_tx_count + 1;
...
task build_expected_response;
    ...
    begin
        ...
        if (resp_status_i == 8'h00) begin
            case (resp_cmd_i)
                8'h02,
                8'h04: payload_count = resp_len_i;
                default: payload_count = 0;
            endcase
        end
        ...
        r_exp_buf[r_exp_count] = resp_len_i[15:8];
        r_chk = r_chk ^ resp_len_i[15:8];
        r_exp_count = r_exp_count + 1;

        r_exp_buf[r_exp_count] = resp_len_i[7:0];
        r_chk = r_chk ^ resp_len_i[7:0];
        r_exp_count = r_exp_count + 1;
...
task verify_axi_full_words_from_data_buf;
    ...
    begin
        ...
        for (j = 0; j < word_count; j = j + 1) begin
            if (u_axi_slave.r_mem_word[base_word + j] !==
                {r_data_buf[j * 4 + 3], r_data_buf[j * 4 + 2], r_data_buf[j * 4 + 1], r_data_buf[j * 4]}) begin
                r_mismatch = 1'b1;
            end
        end

        if (r_mismatch) begin
            $display("[FAIL] %0s: AXI memory content r_mismatch at iter=%0d base_word=0x%0h",
                     case_name, iter_idx, base_word);
            r_fail_count = r_fail_count + 1;
        end
        else begin
            $display("[PASS] %0s: AXI memory content match at iter=%0d base_word=0x%0h",
                     case_name, iter_idx, base_word);
        end
    end
endtask

task run_full_1024_stress;
    integer iter_idx;
    integer base_word_i;
    reg [39:0] req_addr_i;
    integer frame_drop_base;
    begin
        clear_model_memories();
        frame_drop_base = r_frame_drop_count;

        for (iter_idx = 0; iter_idx < 8; iter_idx = iter_idx + 1) begin
            base_word_i = (iter_idx % 4) * 256;
            req_addr_i  = base_word_i * 4;

            $display("[INFO] full_1024_stress: iter=%0d addr=0x%010h write", iter_idx, req_addr_i);
            r_case_count = r_case_count + 1;
            clear_rx_buffer();
            clear_data_buf();
            set_default_slave_ctrl();
            r_full_aw_wait_cycles = iter_idx % 3;
            r_full_w_wait_cycles  = (iter_idx + 1) % 4;
            r_full_b_wait_cycles  = (iter_idx + 2) % 5;
            fill_stress_1024_data(iter_idx);
            build_request(16'd1024, 8'h03, req_addr_i);
            build_expected_response(16'd0, 8'h03, req_addr_i, 8'h00);
            send_tx_buffer();
            wait_for_rx_bytes(r_exp_count, "full_write_1024_stress");
            compare_expected("full_write_1024_stress");
            verify_axi_full_words_from_data_buf(base_word_i, 256, "full_write_1024_stress", iter_idx);

            $display("[INFO] full_1024_stress: iter=%0d addr=0x%010h read", iter_idx, req_addr_i);
            r_case_count = r_case_count + 1;
            clear_rx_buffer();
            set_default_slave_ctrl();
            r_full_ar_wait_cycles = (iter_idx + 1) % 4;
            r_full_r_wait_cycles  = (iter_idx + 2) % 5;
            build_request(16'd1024, 8'h04, req_addr_i);
            build_expected_response(16'd1024, 8'h04, req_addr_i, 8'h00);
            send_tx_buffer();
            wait_for_rx_bytes(r_exp_count, "full_read_1024_stress");
            compare_expected("full_read_1024_stress");
        end

        r_case_count = r_case_count + 1;
        if (r_frame_drop_count != frame_drop_base) begin
            $display("[FAIL] full_1024_stress: unexpected frame_drop pulses, before=%0d after=%0d",
                     frame_drop_base, r_frame_drop_count);
            r_fail_count = r_fail_count + 1;
        end
        else begin
            $display("[PASS] full_1024_stress: no frame_drop pulse across 8x1024B write/read iterations");
        end
    end
endtask
...
initial begin
    ...
    r_data_buf[2] = 8'h34;
    r_data_buf[3] = 8'h12;
    build_request(16'd4, 8'h01, 40'h0000_0000_A0);
    clear_data_buf();
    build_expected_response(16'd0, 8'h01, 40'h0000_0000_A0, 8'h00);
    send_tx_buffer();
    ...
    build_request(16'd4, 8'h02, 40'h0000_0000_A0);
    r_data_buf[0] = 8'h78;
    r_data_buf[1] = 8'h56;
    r_data_buf[2] = 8'h34;
    r_data_buf[3] = 8'h12;
    build_expected_response(16'd4, 8'h02, 40'h0000_0000_A0, 8'h00);
    send_tx_buffer();
    ...
    build_request(16'd16, 8'h03, 40'h0000_0001_00);
    clear_data_buf();
    build_expected_response(16'd0, 8'h03, 40'h0000_0001_00, 8'h00);
    send_tx_buffer();
    ...
    u_axi_slave.r_mem_word[10'h041] = 32'hB0B1_B2B3;
    build_request(16'd8, 8'h04, 40'h0000_0001_00);
    r_data_buf[0] = 8'hA3; r_data_buf[1] = 8'hA2; r_data_buf[2] = 8'hA1; r_data_buf[3] = 8'hA0;
    r_data_buf[4] = 8'hB3; r_data_buf[5] = 8'hB2; r_data_buf[6] = 8'hB1; r_data_buf[7] = 8'hB0;
    build_expected_response(16'd8, 8'h04, 40'h0000_0001_00, 8'h00);
    send_tx_buffer();
    ...
    build_request(16'd256, 8'h03, 40'h0000_0005_00);
    build_expected_response(16'd0, 8'h03, 40'h0000_0005_00, 8'h00);
    send_tx_buffer();
    ...
    build_request(16'd5, 8'h04, 40'h0000_0002_00);
    r_data_buf[0] = 8'hEF; r_data_buf[1] = 8'hBE; r_data_buf[2] = 8'hAD; r_data_buf[3] = 8'hDE; r_data_buf[4] = 8'h78;
    build_expected_response(16'd5, 8'h04, 40'h0000_0002_00, 8'h00);
    send_tx_buffer();
    ...
    build_request(16'd256, 8'h04, 40'h0000_0003_00);
    build_expected_response(16'd256, 8'h04, 40'h0000_0003_00, 8'h00);
    send_tx_buffer();
    ...
    r_full_w_wait_cycles  = 8'd1;
    r_full_b_wait_cycles  = 8'd2;
    for (r_idx = 0; r_idx < 256; r_idx = r_idx + 1) begin
        r_data_buf[r_idx * 4] = r_idx[7:0];
        r_data_buf[r_idx * 4 + 1] = 8'h55 ^ r_idx[7:0];
        r_data_buf[r_idx * 4 + 2] = 8'hAA ^ r_idx[7:0];
        r_data_buf[r_idx * 4 + 3] = 8'hF0 ^ r_idx[7:0];
    end
    build_request(16'd1024, 8'h03, 40'h0000_0008_00);
    build_expected_response(16'd0, 8'h03, 40'h0000_0008_00, 8'h00);
    send_tx_buffer();
    wait_for_rx_bytes(r_exp_count, "full_write_1024b");
    compare_expected("full_write_1024b");
    r_full_mem_mismatch = 1'b0;
    for (r_idx = 0; r_idx < 256; r_idx = r_idx + 1) begin
        if (u_axi_slave.r_mem_word[10'h200 + r_idx] !== {8'hF0 ^ r_idx[7:0], 8'hAA ^ r_idx[7:0], 8'h55 ^ r_idx[7:0], r_idx[7:0]}) begin
            r_full_mem_mismatch = 1'b1;
        end
    end
    if (r_full_mem_mismatch) begin
        $display("[FAIL] full_write_1024b: AXI memory content r_mismatch");
        r_fail_count = r_fail_count + 1;
    end
    ...
    set_default_slave_ctrl();
    for (r_idx = 0; r_idx < 256; r_idx = r_idx + 1) begin
        u_axi_slave.r_mem_word[10'h300 + r_idx] = {8'hC3 ^ r_idx[7:0], 8'hA5 ^ r_idx[7:0], 8'h5A ^ r_idx[7:0], r_idx[7:0]};
        r_data_buf[r_idx * 4] = r_idx[7:0];
        r_data_buf[r_idx * 4 + 1] = 8'h5A ^ r_idx[7:0];
        r_data_buf[r_idx * 4 + 2] = 8'hA5 ^ r_idx[7:0];
        r_data_buf[r_idx * 4 + 3] = 8'hC3 ^ r_idx[7:0];
    end
    build_request(16'd1024, 8'h04, 40'h0000_000C_00);
    build_expected_response(16'd1024, 8'h04, 40'h0000_000C_00, 8'h00);
    send_tx_buffer();
    wait_for_rx_bytes(r_exp_count, "full_read_1024b");
    compare_expected("full_read_1024b");

    run_full_1024_stress();
    ...
    build_request(16'd4, 8'h01, 40'h0000_0000_A3);
    clear_data_buf();
    build_expected_response(16'd0, 8'h01, 40'h0000_0000_A3, 8'h01);
    send_tx_buffer();
    ...
    build_request(16'd6, 8'h03, 40'h0000_0001_00);
    clear_data_buf();
    build_expected_response(16'd0, 8'h03, 40'h0000_0001_00, 8'h03);
    send_tx_buffer();
    ...
    clear_data_buf();
    set_default_slave_ctrl();
    build_request(16'd0, 8'h04, 40'h0000_0000_00);
    clear_data_buf();
    build_expected_response(16'd0, 8'h04, 40'h0000_0000_00, 8'h03);
    send_tx_buffer();
    wait_for_rx_bytes(r_exp_count, "full_read_zero_len_err");
    compare_expected("full_read_zero_len_err");

    r_case_count = r_case_count + 1;
    clear_rx_buffer();
    clear_data_buf();
    set_default_slave_ctrl();
    build_request(16'd0, 8'h07, 40'h0000_0000_00);
    clear_data_buf();
    build_expected_response(16'd0, 8'h07, 40'h0000_0000_00, 8'h04);
    send_tx_buffer();
    ...
    build_request(16'd4, 8'h02, 40'h0000_0000_A0);
    clear_data_buf();
    build_expected_response(16'd0, 8'h02, 40'h0000_0000_A0, 8'h02);
    send_tx_buffer();
    ...
    set_default_slave_ctrl();
    r_full_block_r = 1'b1;
    build_request(16'd4, 8'h04, 40'h0000_0001_00);
    clear_data_buf();
    build_expected_response(16'd0, 8'h04, 40'h0000_0001_00, 8'h02);
    send_tx_buffer();
